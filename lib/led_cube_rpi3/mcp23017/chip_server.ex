defmodule LedCubeRpi3.MCP23017.Server do
  @moduledoc """
  ...
  def init(addr \\ 0x21) do
    Logger.debug("Going to open address #{inspect(addr)}")
    {:ok, handle} = I2C.open(1, addr)
    state = %{handle: handle}
    Logger.debug("Started MCP cube chip on address #{inspect(addr)} ")
    {:ok, state}
  end
  ...
  def handle_cast({:set_mode, pin, mode, pullUp}, state) do
    ...
    Logger.debug("Got to set pin #{inspect(pin)} to mode #{inspect(mode)} on cube chip module")
